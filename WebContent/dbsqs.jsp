...
try {
	...
	//Creating Location Queue
    CreateQueueRequest createLocQueueRequest = new CreateQueueRequest("LOCQueue");
    String myLocQueueUrl = sqs.createQueue(createLocQueueRequest).getQueueUrl();
    //Creating Ant Queue
    CreateQueueRequest createAntQueueRequest = new CreateQueueRequest("ANTQueue");
    String myAntQueueUrl = sqs.createQueue(createAntQueueRequest).getQueueUrl();
    //Creating Ant Queue
   ...
   while(rs.next()){
	   ...
	   sqs.sendMessage(new SendMessageRequest(myLocQueueUrl,requestMsgBdy));
	   sqs.sendMessage(new SendMessageRequest(myAntQueueUrl,requestMsgBdy));
	   sqs.sendMessage(new SendMessageRequest(myBeeQueueUrl,requestMsgBdy));
   }
...
locImpl.runLOC();
//Calling Ant LB
AntColony antImpl= new AntColony();
antImpl.runANT();
//Calling Bee LB
ArtificialBee beeImpl= new ArtificialBee();
beeImpl.runBEE();
